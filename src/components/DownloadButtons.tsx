...
import { Printer, FileText, Share2, Link as LinkIcon } from 'lucide-react';
...
interface DownloadButtonsProps {
  ...
  recordId?: string; // Optional: if record is already saved
}

export function DownloadButtons({ courseInfo, theoryExperiments, programmingSessions, recordId }: DownloadButtonsProps) {
  const handlePrint = () => {
    window.print();
  };
  ...
  const findRecordId = () => {
    ...
    // Find the record by matching course info
  ...
  const handleShareWhatsApp = () => {
    ...
    const message = `Check out my lab record: ${courseInfo.course_title} (${courseInfo.course_code})`;
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      toast.success('Link copied to clipboard!', {
        description: 'Anyone can view this record using the link',
      });
    } catch (error) {
